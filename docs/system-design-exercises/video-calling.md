...
- **No retries.** A video frame or audio packet has a use-by date measured in milliseconds. If it arrives late, it's useless — the frame it belonged to already played (or the ear already needed that audio sample). Retransmitting a lost packet usually means it arrives *after* its deadline, so the sender doesn't bother. Loss is not repaired by asking again; it's **concealed** or it's a visible/audible glitch.
...
Those two facts flip the transport choice: messaging rides TCP (reliable, ordered, will wait) because a chat message *can* wait. Calling rides **UDP**, specifically **RTP** (Real-time Transport Protocol) over UDP, because a video frame *can't* wait — better to drop it and move on than block the whole stream waiting for a retransmit. Everything downstream in this exercise — jitter buffers, forward error correction, SFU vs MCU — is a consequence of committing to "unreliable but on time" over "reliable but possibly late."
...
| Packet loss tolerance | Expected and must be concealed, not retried | A late retransmit is worse than a concealed gap; design for loss as normal operation, not an error case |
...
=== "Packet loss during a call"
    Some percentage of UDP packets are simply lost in transit — this is normal, expected, and must be *concealed*, not retried (a retransmit would arrive after the audio/video frame's deadline anyway). Two complementary techniques: **jitter buffer** — the receiver holds a small buffer (tens of ms) of incoming packets and reorders/paces them for smooth playback, absorbing minor network timing variance; on a truly lost packet, the audio codec conceals the gap (e.g. Opus's packet-loss concealment extrapolates the missing audio from recent samples rather than playing silence) and video either freezes the last good frame briefly or waits for the next keyframe. **Forward error correction (FEC)** — the sender proactively transmits redundant data alongside the stream (e.g. an extra packet that lets the receiver reconstruct one lost packet in a group without needing a retransmit) — a bandwidth-for-resilience trade, tuned to the sender's actual loss rate rather than applied uniformly.
