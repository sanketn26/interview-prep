...
A checkout service calls a separate inventory service to reserve stock. Team Checkout's integration tests use a hand-written mock of the inventory API that returns a successful reservation instantly. Team Inventory ships a **breaking** change — they **rename** `sku` on the reservation **response** (or add a **required request** field `warehouse_id` that checkout does not send). Additive response fields should **not** break consumers. Inventory's own tests pass; checkout starts failing in production within minutes, because the hand-written mock never saw the real contract. Postmortem action: replace the mock with a consumer-driven contract — checkout publishes exactly what fields it reads from the response and what it sends on the request; inventory's CI runs that contract against its real implementation on every change. Additive response fields stay green; removing/renaming a field checkout depends on, or adding a required request field, fails inventory's build before merge.
