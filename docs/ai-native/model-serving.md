...
A 70B-parameter model at FP16 needs roughly 140GB just for weights (2 bytes/parameter × 70B) — too large for a single A100 80GB. Tensor-parallel across **2× A100 80GB** gives 160GB of *nominal* combined GPU memory. **Nominal is not usable-for-weights-and-KV-cache** — real GPU memory also has to cover activations (the intermediate tensors computed during the forward pass, which scale with batch size and sequence length), CUDA context and workspace (typically 1-2GB per GPU just for the driver/runtime), framework allocator overhead (PyTorch/CUDA memory pools reserve more than they immediately use), and fragmentation (the allocator can't always pack memory perfectly, especially with variable-length KV cache entries). A realistic serving stack budgets roughly **85-90% of nominal capacity** as the usable ceiling for weights + KV cache combined — call it ~140GB usable across the 2 GPUs here, not the full 160GB. After the ~140GB of weights, that leaves meaningfully **less than 20GB** for KV cache in practice, not the full nominal remainder — the exact number depends on the serving framework's overhead, but treating 160GB as fully available for weights+cache is the specific overclaim to avoid.

KV cache cost per token (a 80-layer, 8-KV-head, 128-head-dim architecture, FP16): `2 (K and V) × 80 layers × 8 KV heads × 128 head-dim × 2 bytes ≈ 320KB per token`. At an average context length of ~2,048 tokens in flight per request, one request's KV cache costs `2,048 × 320KB ≈ 640MB`. A **batch size of 32** would need `32 × 640MB ≈ 20GB` of KV cache — which is roughly the *nominal* leftover after weights, but not a safe real-world number once activations, CUDA workspace, and fragmentation are accounted for. Treat batch 32 as the number this specific memory math suggests *before* subtracting real-world overhead, not as a validated practical ceiling — the actual safe batch size needs to be confirmed against measured GPU memory usage under load (e.g., `nvidia-smi` during a load test, or the serving framework's own memory profiler), not derived from weights + KV cache alone. A framework like vLLM that manages KV cache via paged allocation will report its own safe ceiling; use that number over a back-of-envelope one when it's available.
