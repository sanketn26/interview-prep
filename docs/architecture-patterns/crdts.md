...
**RGA (Replicated Growable Array):** the CRDT behind collaborative text editing (ordered sequences — think Google Docs-style character insertion, though Docs itself uses Operational Transformation, not RGA). Each element has a unique ID and a reference to its logical predecessor; concurrent inserts at the same position are ordered deterministically by ID, so "type X" and "type Y" at the same cursor position from two users both survive, ordered consistently across all replicas, rather than one overwriting the other.
