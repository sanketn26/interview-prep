...
- **Serverless (Lambda + S3 trigger):** function fires on S3 upload event, generates thumbnails, writes back to S3. At 50K uploads/day averaging ~0.6/sec but bursting to 2000/sec during a marketing push, Lambda scales to match the burst automatically — no pre-provisioning. Cost: 0.5GB × 0.8s = 0.4 GB-seconds per invocation × $0.0000166667/GB-sec ≈ $6.67e-6 per invocation × 50K invocations/day ≈ $0.33/day ≈ **$10/month** compute. Add request-count charges (AWS Lambda: ~$0.20 per million requests, so ~$0.30/month at this volume) and the total is still roughly $10/month. A fleet of always-on containers sized for the 2000 req/sec burst would idle at near-zero utilization 99% of the day and cost far more for the same throughput.
